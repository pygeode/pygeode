...
def buildGFS():
  yr = 2019
  mn = 2
  tmp = "https://www.ncei.noaa.gov/thredds/dodsC/gfs-g4-anl-files/{year}{month:02d}/{year}{month:02d}{day:02d}/gfsanl_4_{year}{month:02d}{day:02d}_0000_000.grb2"
  urls = [tmp.format(year = yr, month = mn, day = d) for d in range(20, 28)]

  namemap = { 'u-component_of_wind_isobaric' : 'u',
  ...
  ds = []
  for u in urls:
    d = pyg.open(u, format = 'netcdf4', dimtypes = dict(isobaric = pyg.Pres), \
                          namemap = namemap, varlist = list(namemap.keys()))
    ds.append(d)

  
  return pyg.concatenate(ds)
