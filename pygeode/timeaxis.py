...
# {{{
    ...
    fnames = set(self.auxarrays.keys())
    ...
    if resolution is not None:
      ...
      fnames -= set(self.allowed_fields[i+1:])

    # Convert back to an ordered list
    fnames = [f for f in self.allowed_fields if f in fnames]

    # Get the fields
    fields = [self.auxarrays[f] for f in fnames]
    ...
    kwargs = dict(self.auxatts, **fields)
...
# }}}
  ...
  def reltime (self, startdate=None, units=None):
    ...
    return self.date_as_val (startdate=startdate, units=units)
  ...
  # {{{
    ...
    delt = np.diff(self.reltime(units=units))
  ...
  # {{{
    return self.val_as_date(self.date_as_val(dt), allfields=allfields)
  # }}}

  # Helper function; returns time between two dates in specified units
  def date_diff(self, dt1, dt2, units = None):
  # {{{
    return self.date_as_val(dt2, startdate=dt1, units = units)
  # }}}
